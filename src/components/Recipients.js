...
const Recipients = () => {
  ...
  const addRecipient = async () => {
    try {
      ...
      const tx = await shareContract.addRecipient(
        recipientAddress,
        recipientName
      );
      await tx.wait();
  ...
  const [recipientAddress, setRecipientAddress] = useState(""); //for reciepient component different fron in app.js
  ...
  useEffect(() => {
    async function getData() {
      ...
      const recipients = await shareContract.filters.recipeints(App.address);
      const recipentsData = await shareContract.queryFilter(recipients);
